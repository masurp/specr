...
For this example, we will us the `gapminder` data set that is included in the [gapminder](https://cran.r-project.org/web/packages/gapminder/index.html) package. We quickly recode some variable to get more interpretable estimates.
...
For this example, we use the package [lme4](https://cran.r-project.org/web/packages/lme4/index.html) and more specifically the function `lmer()` to estimate the multilevel model (more complex models such as poisson or negative binomial multilevel models can likewise be estimated). 
