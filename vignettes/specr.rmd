...
The following example is a more comprehensive version of the example on the homepage and exemplifies how to use the major functions of this package. Although using `specr` strictly speaking does not require any other package, we recommend to also load the `tidyverse` as it provides valuable functions for data wrangling and adapting outputs from `specr`-functions. Furthermore, the `tidyverse` includes `ggplot2` which can be used to modify the layout of the plot outputs. 
...
The resulting data frame includes relevant statistics for each of the estiamted models. 
